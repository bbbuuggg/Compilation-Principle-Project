...
public class SymbolTable {
    ...
    private static final Map<String, SymbolTableEntry> symbol_text = new HashMap<>();
    /**
     * 获取符号表中已有的条目
     *
     * @param text 符号的文本表示
    ...
    public SymbolTableEntry add(String text) {
        if(has(text)){
            ...
            SymbolTableEntry symbol = new SymbolTableEntry(text);
            symbol_text.put(text,symbol);
            return symbol;
        }
    }
    ...
    public boolean has(String text) {
        return symbol_text.containsKey(text);
    }
    ...
    private Map<String, SymbolTableEntry> getAllEntries() {
        return Collections.unmodifiableMap(symbol_text);
    }
